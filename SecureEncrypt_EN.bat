...
set "_V_SIG=31283fa02c2208fa3947b9f864c215207a95c07e20ceff1821a856928bba5300"
...
if "%~1"=="" (
    ...
    echo.
    echo.
    echo You can also use it without installation.
    echo.
    echo Please drag and drop a file onto this script file to encrypt.
    echo.
    ...
    if exist "C:\Windows\SecureEncrypt.cmd" (
        ...
        if !errorlevel! equ 1 (
            echo.
            echo Installing...
			reg delete "HKEY_CLASSES_ROOT\*\shell\SecureEncode" /f >nul 2>nul
			del /f /q C:\Windows\SecureEncrypt.cmd 2>nul
            copy /y "%~f0" "C:\Windows\SecureEncrypt.cmd" >nul
            reg add "HKCR\*\shell\SecureEncrypt" /ve /d "Encrypt File (SecureEncrypt AES-256)" /f >nul
            ...
            echo.
            echo Right-click menu added: "Encrypt File (SecureEncrypt AES-256)"
            echo.
        ) else (
            echo.
...
echo (If you don't enter a password, the file will ONLY be packed as binary data (RAW), NOT ENCRYPTED)
echo.
...
if defined user_pass (
    ...
    echo.
    echo Password not entered. The file will NOT be encrypted.
)
echo.
...
echo echo. >> "%output_cmd%"
echo echo ================== DECRYPTION PROCESS ================== >> "%output_cmd%"
...
:: === Write PowerShell parser line by line - STREAM BASED ===
...
echo echo ^    # STREAM-BASED READING - Find the marker ^>^> "%%TEMP_PS%%" >> "%output_cmd%"
...
echo echo ^            if ^(^$readCount -ne 32^) ^{ throw ^"File corrupted (Sentinel unreadable)^" ^} ^>^> "%%TEMP_PS%%" >> "%output_cmd%"
...
echo Decrypt script created successfully.
echo.
