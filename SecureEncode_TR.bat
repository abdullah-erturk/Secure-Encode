...
set "_V_SIG=50ef42b5ab33bafcbb93c9a7e63d4376dbf1420b3cab51a30b73673a900f9896"
...
echo echo ^	if^(Test-Path ^$outPath^)^^{ ^$sha^^^=^(Get-FileHash -LiteralPath ^$outPath -Algorithm SHA256^).Hash^; Write-Host ^"Hesaplanan SHA256: ^$sha^"^; if^(^$sha -ieq ^$env:expected_sha^)^^{ Write-Host ^"SHA256: BASARILI^" ^} else ^{ Write-Host ^"SHA256: BASARISIZ^" ^} ^} ^>^> "%%TEMP_PS%%" >> "%output_cmd%"
echo echo ^} catch [System.Security.Cryptography.CryptographicException] ^{ ^>^> "%%TEMP_PS%%" >> "%output_cmd%"
echo echo ^	if^(Test-Path ^$outPath^)^^{ Remove-Item ^$outPath -ErrorAction SilentlyContinue ^} ^>^> "%%TEMP_PS%%" >> "%output_cmd%"
echo echo ^	exit 1 ^>^> "%%TEMP_PS%%" >> "%output_cmd%"
echo echo ^} catch ^{ ^>^> "%%TEMP_PS%%" >> "%output_cmd%"
