...
set "_V_SIG=bc2cde48912a140cd40ac9af35fa06234ae12a5def2cb07c09f9082b9a9fd8dc"
...
set "user_pass="
for /f "delims=" %%p in ('powershell -ExecutionPolicy Bypass -NoProfile -Command "$securePass=Read-Host -AsSecureString -Prompt 'Password (Press ENTER to skip encryption)'; $bstr=[System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($securePass); $password=[System.Runtime.InteropServices.Marshal]::PtrToStringAuto($bstr); [void][System.Runtime.InteropServices.Marshal]::ZeroFreeBSTR($bstr); [void]$securePass.Dispose(); Write-Output $password"') do (
    set "user_pass=%%p"
)
...
for /f "delims=" %%S in ('powershell -NoProfile -Command "$s=%src_size%; if($s -ge 1GB){'{0:N2} GB' -f ($s/1GB)}elseif($s -ge 1MB){'{0:N2} MB' -f ($s/1MB)}else{'{0:N2} KB' -f ($s/1KB)}"') do set "SIZE_STR=%%S"
...
echo echo ^	if^(Test-Path ^$outPath^)^^{ ^$sha^^^=^(Get-FileHash -LiteralPath ^$outPath -Algorithm SHA256^).Hash^; Write-Host ^"Calculated SHA256: ^$sha^"^; if^(^$sha -ieq ^$env:expected_sha^)^^{ Write-Host ^"SHA256: SUCCESS^" ^} else ^{ Write-Host ^"SHA256: FAILED^" ^} ^} ^>^> "%%TEMP_PS%%" >> "%output_cmd%"
echo echo ^} catch [System.Security.Cryptography.CryptographicException] ^{ ^>^> "%%TEMP_PS%%" >> "%output_cmd%"
echo echo ^	if^(Test-Path ^$outPath^)^^{ Remove-Item ^$outPath -ErrorAction SilentlyContinue ^} ^>^> "%%TEMP_PS%%" >> "%output_cmd%"
echo echo ^	exit 1 ^>^> "%%TEMP_PS%%" >> "%output_cmd%"
echo echo ^} catch ^{ ^>^> "%%TEMP_PS%%" >> "%output_cmd%"
